...
export default function App() {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'process':     return <Process setCurrentPage={setCurrentPage} />;
      case 'testimonials':return <Testimonials setCurrentPage={setCurrentPage} />;
  ...
  return (
    <>
      {/* Intro sits on top (z-9999, fixed), handles its own slide-up exit */}
      {introPlaying && (
        <FullScreenIntro onComplete={() => setIntroPlaying(false)} />
      )}

      {/* Main website — always rendered underneath, animates in once */}
      <motion.div
        initial={{ opacity: 0, scale: 0.98 }}
        animate={{ opacity: introPlaying ? 0 : 1, scale: introPlaying ? 0.98 : 1 }}
